function plot_traffic(id, todays_data, yesterdays_data, monthly_average) {
    ...
    var height=$(target).height() - top_gutter;
    ...
    var maxValue = d3.max([].concat(todays_data).concat(yesterdays_data).concat(monthly_average));
    ...
    var tick_values = get_tick_values(maxValue, 6).map(Math.ceil);
    yAxis = d3.svg.axis().scale(yscale_for_axis)
        .orient("left")
        .tickValues(tick_values)
        .tickFormat(function (each) { return format_tick_label(each, maxValue); });
...
function get_tick_values(maxValue, numberOfTicks) {
    var upper = maxValue;
    var step = upper/(numberOfTicks - 1);
...
function get_data_to_plot(todays_data, yesterdays_data) {
    var combined = [];
    var i = 0;
    for (i = 0; i < todays_data.length; i++) {
        combined.push(todays_data[i])
    }
    for (; i < yesterdays_data.length; i++) {
        combined.push(yesterdays_data[i])
    }
    return combined;
}
...
function format_tick_label(tick_value, max_value)
{
    if (max_value >= 1000000) {
        return Math.round(tick_value/1000000) + "m";
    }
    if (max_value >= 1000) {
        return Math.round(tick_value/1000) + "k";
    }
    return "" + tick_value;
}
