function plot_traffic(id, todays_data, yesterdays_data, monthly_average) {
    ...
    var tick_values = get_tick_values(data_to_plot, 6).map(Math.ceil);
    yAxis = d3.svg.axis().scale(yscale_for_axis)
        .orient("left")
        .tickValues(tick_values)
        .tickFormat(function (each) { return format_tick_label(each, d3.max(data_to_plot)); });
...
function get_tick_values(data, numberOfTicks) {
    var upper = d3.max(data);
