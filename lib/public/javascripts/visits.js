...
GOVUK.Insights.sixMonthTimeSeries = function(url, target, params) {
    ...
    var series = extractKeys(params.series),
        margins = params.margins || [22, 27, 15, 54],
        ...
        render: function() {
            d3.json(url, function(data) {
                var alldata = concat(data, series),
                    xExtent = d3.extent(alldata, function(d) { return dateFormat.parse(d.date); }),
                ...
                /* Label Placement */
                var seriesLastValue = $(series).map(function(i, name) {
                ...
                function createTextLabel(item, ypos) {
                    graph.append("svg:text")
                        ...
                        .attr("y", ypos)
                        .text(item.legend);
                    $("#" + item.name + "-label").attr("x", width - margins[3]);
                }

                // place bottom legend
                var legendHeight = 20;
                if ((seriesLastValue[0].ypos - seriesLastValue[1].ypos) < legendHeight) {
                ...
                // place the top one
                if (seriesLastValue[2].ypos > legendHeight) {
